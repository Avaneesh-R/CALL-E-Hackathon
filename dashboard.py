...
.callie-orb{width:54px;height:54px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#2563eb);box-shadow:0 4px 24px rgba(124,58,237,.45);display:flex;align-items:center;justify-content:center;transition:transform .2s,box-shadow .2s;position:relative}
.callie-orb:hover{transform:scale(1.08);box-shadow:0 8px 32px rgba(124,58,237,.65)}
...
<!-- ── New Campaign Wizard ── -->
<div class="nc-bar">
  <button class="nc-open-btn" id="nc-open-btn" onclick="toggleWizard()">&#43; New Campaign</button>
</div>
...
function callieToggle() {
  ...
  if (_callieOpen) {
    ...
    document.getElementById('callie-orb').classList.remove('pulse');
    ...
    if (!_callieBiz) {
      ...
      else setTimeout(() => document.getElementById('callie-input').focus(), 80);
    }
  } else {
...
function callieAddMsg(role, text, html) {
  _callieMsgs.push({ role, text });
  const msgs = document.getElementById('callie-messages');
  const now = new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
  const div = document.createElement('div');
  div.className = 'c-msg ' + role;
  const star = `<svg width="12" height="12" viewBox="0 0 24 24" fill="rgba(255,255,255,.9)"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>`;
  const content = html || _callieMd(role === 'user' ? escHtml(text) : text);
  if (role === 'callie') {
    div.innerHTML = `<div class="c-mini-avatar">${star}</div><div><div class="c-bubble">${content}</div><div class="c-time">${now}</div></div>`;
  } else {
...
function callieShowTyping() {
  ...
  div.innerHTML=`<div class="c-mini-avatar">${star}</div><div class="c-typing-bubble"><span></span><span></span><span></span></div>`;
...
function callieQuick(text){callieAddMsg('user',text);callieRequest(text);}
...
async function callieRequest(msg){
  document.getElementById('callie-send-btn').disabled=true;
  callieShowTyping();
  try{
    const res=await fetch('/api/chat',{
      method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({message:msg,history:_callieMsgs.slice(-12),business_name:_callieBiz})
    });
    const data=await res.json();
    callieHideTyping();
    if(data.reply) callieAddMsg('callie',data.reply,data.reply_html);
    ...
    document.getElementById('callie-send-btn').disabled=false;
    setTimeout(()=>document.getElementById('callie-input').focus(),50);
  }
}

function callieExecAction(a){
  if(a.type==='navigate_to'){switchTab(a.tab);callieToast('Navigated to '+a.tab+' tab');}
...
// Pulse callie button on new data
setInterval(()=>{
  if(!_callieOpen){
    document.getElementById('callie-orb').classList.add('pulse');
  }
},60000);
</script>

<!-- ── Callie Floating Avatar ── -->
<button class="callie-btn" onclick="callieToggle()" id="callie-btn" title="Chat with Callie, your AI assistant">
  <div class="callie-orb pulse" id="callie-orb">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" fill="rgba(255,255,255,.9)" stroke="rgba(255,255,255,.3)" stroke-width=".5"/></svg>
...
<div class="callie-panel" id="callie-panel">
  ...
  <div class="callie-header">
    <div class="callie-avatar-sm">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="rgba(255,255,255,.9)"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>
    </div>
    <div>
  ...
  <div id="callie-setup-screen" class="callie-setup">
    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" style="margin-bottom:4px"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" fill="url(#cg)" stroke="rgba(124,58,237,.3)" stroke-width=".5"/><defs><linearGradient id="cg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#7c3aed"/><stop offset="100%" stop-color="#2563eb"/></linearGradient></defs></svg>
    <h3>Hi, I'm Callie</h3>
    <p>Your AI outreach partner with full access to your CALL-E pipeline — vendor discovery, calling campaigns, WhatsApp outreach, and real-time results.</p>
    ...
    <button class="callie-setup-btn" onclick="callieSetupDone()">Let's start &#8594;</button>
  </div>
  <!-- Messages -->
...
def _callie_build_context():
    ...
    try:
        ...
        return (
            f"Campaigns: {camp_list}\n"
            f"Lead totals: {s['total']} total | {s['pos'] or 0} positive | "
...
def _callie_tools():
    return [
        {"type": "function", "function": {
            ...
            "description": "Search for vendors in a city/area by product or category using OpenStreetMap. Creates a new campaign and returns a scored vendor list.",
            "parameters": {"type": "object", "properties": {
                "product":     {"type": "string", "description": "What to source or which business type, e.g. 'restaurants', 'pharmacies', 'office furniture'"},
                "location":    {"type": "string", "description": "City or area, e.g. 'Connaught Place, Delhi'"},
                "max_vendors": {"type": "integer", "description": "Max vendors to find (default 15, max 30)"},
            }, "required": ["product", "location"]}
        }},
        {"type": "function", "function": {
            "name": "start_campaign",
            "description": "Start the AI voice calling pipeline for an existing campaign.",
            "parameters": {"type": "object", "properties": {
                "campaign_id": {"type": "integer", "description": "Campaign ID to start"},
                "language":    {"type": "string", "description": "Call language: Hindi, English, or Hindi and English"},
            }, "required": ["campaign_id"]}
        }},
        {"type": "function", "function": {
            "name": "get_campaigns",
            "description": "Get summary of all campaigns with lead counts and call status breakdown.",
            "parameters": {"type": "object", "properties": {}}
        }},
        {"type": "function", "function": {
            "name": "get_leads",
            "description": "Get leads with optional filters. Use to show positive/negative/pending vendors.",
            "parameters": {"type": "object", "properties": {
                "campaign_id": {"type": "integer", "description": "Campaign ID (omit = all campaigns)"},
                "status":      {"type": "string", "description": "Filter: positive, negative, no_answer, not_called, failed"},
                "limit":       {"type": "integer", "description": "Max results (default 20)"},
            }}
        }},
        {"type": "function", "function": {
            "name": "send_whatsapp",
            "description": "Send a WhatsApp text message to a vendor or contact.",
            "parameters": {"type": "object", "properties": {
        ...
        {"type": "function", "function": {
            "name": "check_whatsapp",
            "description": "Check WhatsApp connection status and view recent inbox messages.",
            "parameters": {"type": "object", "properties": {
                "from_number": {"type": "string", "description": "Filter inbox by phone number (optional)"},
            }}
        }},
        {"type": "function", "function": {
            "name": "navigate_dashboard",
            "description": "Navigate the client's dashboard to a tab or open the campaign wizard.",
            "parameters": {"type": "object", "properties": {
        ...
        {"type": "function", "function": {
            "name": "get_call_stats",
            "description": "Get overall pipeline statistics: totals, positive rate, campaign count.",
            "parameters": {"type": "object", "properties": {}}
        }},
    ]
...
def api_chat():
    ...
    if not api_key:
        return jsonify({"reply": "⚠️ GROQ_API_KEY environment variable not set. Please restart the dashboard after setting it."}), 200

    ctx = _callie_build_context()

    system = f"""You are Callie, an elite AI outreach assistant working for **{biz_name}**.
You speak directly to the business owner/operator in a warm, sharp, professional tone — like a senior BD manager who works for their company.

== CALL-E SYSTEM KNOWLEDGE ==
CALL-E is an AI-powered vendor discovery and outreach system built for {biz_name}:
- Vendors are discovered via OpenStreetMap, scored 1-10 for fit, then contacted via AI voice calls
- Call Round 1 (R1): initial discovery call to gauge interest and gather info
- Call Round 2 (R2): follow-up call to interested (positive R1) vendors
- Statuses: not_called → positive / negative / no_answer / completed / failed
- WhatsApp outreach runs in parallel via a Baileys server on localhost:3001
- Campaigns group vendors discovered for a specific product+location search

== LIVE PIPELINE DATA ==
{ctx}

== YOUR CAPABILITIES ==
Use the provided tools to take REAL action. Never say "I can't" — use the tools:
- discover_vendors → find & save vendors from OpenStreetMap, creates a campaign automatically
- start_campaign → kick off the AI voice calling pipeline for any campaign
- get_campaigns → full breakdown of all campaigns
- get_leads → filter leads by status/campaign
- send_whatsapp → send a WhatsApp message to any number
- check_whatsapp → connection status + inbox
- navigate_dashboard → switch tabs, open wizard
- get_call_stats → overall stats

== RULES ==
1. Always use tools to get current data — never fabricate numbers
2. After discover_vendors, tell the client the campaign ID and offer to start calling
3. After start_campaign, tell them to watch the Live tab
4. Refer to vendors by name when possible
5. Never modify the codebase — your control is entirely through the provided tools
6. Be specific: names, numbers, percentages — not vague summaries
7. If WhatsApp is offline, say so clearly and explain they need to scan the QR code
"""
    ...
    for _ in range(5):
        ...
        try:
            ...
            return jsonify({"reply": f"⚠️ Groq API error {e.code}: {body[:200]}"}), 200
        except Exception as e:
            return jsonify({"reply": f"⚠️ Request failed: {e}"}), 200
    ...
    return jsonify({"reply": "I ran into a loop — could you rephrase your request?", "actions": actions}), 200
